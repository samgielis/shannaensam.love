        ...
        <CardBody>
          <Stack spacing={4}>
            <FormControl isRequired>
              ...
              <FormHelperText>
                {trans(
                  translations.invitation.step2.inputs.email.helpText,
                  language
                )}
              </FormHelperText>
            </FormControl>
            ...
            <Hideable hidden={!invitation.plusOne}>
              <FormControl isRequired>
                <FormLabel>
                  {trans(
                    translations.invitation.step2.inputs.amount.label,
                    language
                  )}
                </FormLabel>
                <NumberInput
                  ...
                  onChange={(_, valueAsNumber) => {
                    setAmountOfPeople(
                      Math.min(Math.max(valueAsNumber || 0, 1), 10)
                    );
                    setTimeout(updateValidity);
                  }}
                >
                  <NumberInputField
                    onChange={() => setTimeout(updateValidity)}
                  />
                  <NumberInputStepper>
                    <NumberIncrementStepper
              ...
              <Stack>
                {Array.from(Array(amountOfPeople).keys()).map((index) => (
                  <Input
                    ...
                    name="name"
                    placeholder={`${trans(
                      translations.invitation.step2.inputs.listOfPeople
                        .placeholder,
                      language
                    )} ${index + 1}`}
                    isRequired
